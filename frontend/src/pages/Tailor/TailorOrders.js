...
const TailorOrders = () => {
  ...
  const get_rating = async (type, id) => {
    try {
      const response = await axios.get(`/api/g/rating/${type}/${id}`)
      set_customer_bearing(response.data.rating)
    } catch (e) {
      console.log(e)
    }
  }
  ...
  const [customer_bearing, set_customer_bearing] = useState(0)
  ...
  const order = async (_id, n) => {
    try {
      const response = await axiosPrivate.
        patch(`/api/tailor/orders/product/`, { _id: _id, status: n }
        )
      set_reload(p => !p)
                  ...
                  <Col xs={12}>
                    <Card >
                        ...
                        <Row>
                          <Col>
                            ...
                            <Card.Text>
                              ...
                              <br />
                              <Button variant='info' as={Link} to={`/shops/${i.shop_id}`} className='m-3'>Go to Shop</Button>
                              <Button variant='info' as={Link} to={`/products/${i.shop_id}/${i.product_id}`} className='m-3'>Go to Product</Button>
                            </Card.Text>
                          </Col>
                          <Col>
                            <Table responsive={true}>
                              <tbody>
                                      ...
                                      (i.status === -1) ? <>
                                        ...
                                        (i.status === 0) ? <>
                                          ...
                                          (i.status === 1) ? <>
                                            ...
                                            (i.status === 2) ? <>
                                              <h6>
                                                Order has been ammended by you, waiting for respond to changes
                                              </h6>
                                            </>
                                              :
                                              (i.status === 3) ? <>
                                                <h6>
                                                  customer ammended the order
                                                </h6>
                                              </>
                                                :
                                                (i.status === 4) ? <>
                                                  ...
                                                  (i.status === 5) ? <>
                                                    ...
                                                    (i.status === 6) ? <>
                                                      <h6>
                                                        customer have accepted the changes
                                                      </h6>
                                                    </>
                                ...
                                <tr>
                                  <td>Customer brearing</td>
                                  <td>{(customer_bearing < 0) ?
                                    <h4>
                                      <Badge bg="danger">Risky</Badge>
                                    </h4> :
                                    (customer_bearing > 0) ?
                                      <h4>
                                        <Badge bg="success">Trustable Customer</Badge>
                                      </h4> :
                                      (customer_bearing === 0) ?
                                        <h4>
                                        ...
                                        <h4>
                                          <Badge bg="warning">erro when taking rating</Badge>
                                        </h4>
                                  }</td>
                                </tr>
                      ...
                      <Card.Footer>
                        <Container>
                          <Row>
                            <Col xs={6} sm={'auto'} className='m-1'>
                              <Button hidden={!(i.status === 0 || i.status === 2 || i.status === 3)}
                                variant='danger'
                                onClick={() => { order(i._id, -1) }}
                              >
                                Cancel Order
                              </Button>
                            </Col>
                            <Col xs={6} sm={'auto'} className='m-1'>
                              <Button hidden={!(i.status === 2 || i.status === 0 || i.status === 3)}
                                variant='warning' >
                                Amend Order</Button>

                            </Col>
                            <Col xs={6} sm={'auto'} className='m-1'>
                              <Button hidden={!(i.status === 6 || i.status === 0 || i.status === 1 || i.status === 3)}
                                variant='warning'
                                onClick={() => { order(i._id, 4) }}
                              >
                                Set as Delivering</Button>
                            </Col>
                            <Col xs={6} sm={'auto'} className='m-1'>
                              <Button hidden={!(i.status === 4)}
                                variant='warning'
                                onClick={() => { order(i._id, 5) }}
                              >
                                Set as Completed</Button>
                            </Col>
                            <Col xs={6} sm={'auto'} className='m-1'>
                              <Button hidden={!(i.status === 0)}
                                variant='success'
                                onClick={() => { order(i._id, 1) }}
                              >
                                Accept Order</Button>
