...
const ShopPageTailor = () => {
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const da_response = await axiosPrivate.get(`/api/tailor/shop/si/${id}`)
    ...
    <Container className='my-3 py-3' >
      ...
      <Row className='justify-content-evenly my-3'>
        <Col className='mx-auto'>
          <Button variant='danger' onClick={() => setrefresh(Prev => !Prev)}>ShutDown Shop</Button>
        </Col>
                      ...
                      <Container className='justify-content-center text-center'>
                        <Row>
                          <Col xs={12} sm={true} className='my-2'>
                            <Button variant="warning" as={Link} to={`/shops/edit/${id}`}>
                              EDIT INFO
                            </Button>
                          </Col>
                          <Col xs={12} sm={true} className='my-2'>
                            <Button variant="info" as={Link} to={`/shops/add-product/${id}`}>ADD PRODUCT</Button>
                          </Col>
                          <Col xs={12} sm={true} className='my-2'>
                            <Button variant="info" as={Link} to={`/shops/add-service/${id}`}>ADD SERVICE</Button>
                          </Col>
                          {(response.status === 0) ?
                            <Col xs={12} sm={true} className='my-2'>
                              <Button variant="danger" as={Link} to={`/shops/request-activation/${id}`}>REQUEST ACTIVATION</Button>
                            </Col>
                            :
                            <></>
                          }
