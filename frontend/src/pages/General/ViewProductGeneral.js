...
const ViewProductGeneral = () => {
  ...
  const add_to_cart = (item) => {
    ...
    // console.log(order_negotiate === 1, order_negotiation_offer === 0, order_quantity === 0)
    const order_detail = {
      price: response.product_price - (response.product_price * (response.discount / 100)),
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const da_response = await axios.get(`/api/g/product/pi/${id}`)
                    ...
                    <Container className='my-2 py-2'>
                      <Row>
                        <Modal show={bulk_buy_modal} onHide={() => set_bulk_buy_modal(false)}>
                          ...
                          <Modal.Body>Buying in Bulk can allow you to buy the product in great quantity, it is also possible to have product for wholesale price if the supplier is willing to lower the price</Modal.Body>
                            ...
                            <Form>
                              <Container>
                                ...
                                <Row className='m-1'>
                                  ...
                                  <Col>
                                    <InputGroup >
                                      <Form.Control readOnly type='number' min={1} className='w-50' value={response.product_price - (response.product_price * (response.discount / 100))} />
                                ...
                                {(response.product_options.includes(1)) ?
                                  <>
                                    <Row className='m-1'>
                                      <Col>
                      ...
                      <Row>
                            ...
                            <Row>
                              <Accordion defaultActiveKey="0" flush>
                                <Accordion.Item eventKey="0">
                                  <Accordion.Header ><h1>PRODUCT DESCRIPTION</h1></Accordion.Header>
                                  <Accordion.Body>
                        ...
                        <Col sm={12} lg={4}>
                          ...
                          <br />
                          {(response.product_categories && (response.product_categories.gender || response.product_categories.age_group || response.product_categories.article_type || response.product_categories.season || response.product_categories.wear_type || response.product_categories.other_tags.length !== 0)) ? <>
                            <h3>
                              PRODUCT CATEGORIES
                            </h3>
                            <Container>
                              <Row>
                                {(response.product_categories && response.product_categories.gender && response.product_categories.gender !== '-') ?
                                  <Col xs={3}>
                                    <h5>
                                      <Badge bg='success' >
                                        {response.product_categories.gender}
                                      </Badge>
                                    </h5>
                                  </Col>
                                  : <></>}
                                {(response.product_categories && response.product_categories.age_group && response.product_categories.age_group !== '-') ?
                                  <Col xs={3}>
                                    <h5>
                                      <Badge bg='success' >
                                        {response.product_categories.age_group}
                                      </Badge>
                                    </h5>
                                  </Col>
                                  : <></>}
                                {(response.product_categories && response.product_categories.article_type && response.product_categories.article_type !== '-') ?
                                  <Col xs={3}>
                                    <h5>
                                      <Badge bg='success' >
                                        {response.product_categories.article_type}
                                      </Badge>
                                    </h5>
                                  </Col>
                                  : <></>}
                                {(response.product_categories && response.product_categories.season && response.product_categories.season !== '-') ?
                                  <Col xs={6}>
                                    <h5>
                                      <Badge bg='success' >
                                        {'Season: ' + response.product_categories.season}
                                      </Badge>
                                    </h5>
                                  </Col>
                                  : <></>}
                                {(response.product_categories && response.product_categories.wear_type && response.product_categories.wear_type !== '-') ?
                                  <Col xs={6}>
                                    <h5>
                                      <Badge bg='success' >
                                        {'Wear: ' + response.product_categories.wear_type}
                                      </Badge>
                                    </h5>
                                  </Col>
                                  : <></>}
                                <br />
                                {(response.product_categories && response.product_categories.other_tags && response.product_categories.other_tags.length !== 0) ?
                                  <>
                                    {response.product_categories.other_tags.map((i, n) => {
                                      return (
                                        <Col key={n} xs='auto'>
                                          <h5>
                                            <Badge bg='success' >
...
export default ViewProductGeneral