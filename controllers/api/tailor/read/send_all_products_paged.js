...
const send_all_products_paged = async (req, res) => {
    try {
        ...
        results = await product.find({ status: 1, visibility: 1 }, ["-updatedAt", "-__v", "-tailor_id", "-product_description", "-product_options", "-payment_methods", "-ordering_methods"]).skip((page - 1) * 10).limit(10)
